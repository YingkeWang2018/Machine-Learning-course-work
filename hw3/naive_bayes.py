...
def train_mle_estimator(train_images, train_labels):
    ...
    theta_mle = np.divide(np.matmul(train_images.transpose(), train_labels), np.matmul(np.ones((1, train_labels.shape[0])), train_labels))
...
def train_map_estimator(train_images, train_labels):
    ...
    theta_map = np.divide(np.matmul(train_images.transpose(), train_labels) + 2, np.matmul(np.ones((1, train_labels.shape[0])), train_labels) + 4)
...
def log_likelihood(images, theta, pi):
    ...
    # YOU NEED TO WRITE THIS PART
    top1 = np.matmul(images, np.log(theta)) + np.matmul(1 - images, np.log(1 - theta)) + np.log(pi)
    bottom_first = top1 - np.log(pi)
    bottom1 = np.log(np.matmul(np.exp(bottom_first), pi)).reshape((len(bottom_first), 1))
    return top1 - bottom1
...
def predict(log_like):
    ...
    # YOU NEED TO WRITE THIS PART
    predictions = np.argmax(log_like, axis=1).reshape(-1, 1)
...
def accuracy(log_like, labels):
    ...
    return np.mean(predictions == np.argmax(labels, axis=1).reshape(-1, 1))
